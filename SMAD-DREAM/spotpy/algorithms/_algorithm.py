...
class _algorithm(object):
    ...
    def set_repetiton(self, repetitions):
        self.status = _RunStatistic(repetitions, self.algorithm_name, self.optimization_direction, self.parnames)
    ...
    def simulate(self, id_params_tuple, SurrogateModel=None, scaler_param=None, scaler_result=None):
        ...
        if SurrogateModel is not None:
            # 将 all_params 转换为 NumPy 数组
            all_params_np = np.array(all_params)
            all_params_np = all_params_np.reshape(1, -1)
            param_scaled = scaler_param.transform(all_params_np)
            # 将 NumPy 数组转换为 PyTorch 张量
            all_params_tensor = torch.FloatTensor(param_scaled).unsqueeze(1)
            model_result = self.get_output(SurrogateModel, all_params_tensor)
            model_result = model_result.cpu()  # 将张量从 GPU 移动到 CPU
            model_result = scaler_result.inverse_transform(model_result)
            model_result = model_result.flatten()  # 将 model_result 转换为一维数组
        else:
