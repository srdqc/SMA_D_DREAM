...
class sceua(_algorithm):
    ...
    def simulate(self, id_params_tuple, SurrogateModel=None, scaler_param=None, scaler_result=None):
        ...
        if not self.repeat.phase:  # burn-in
            return _algorithm.simulate(self, id_params_tuple, SurrogateModel=None, scaler_param=None, scaler_result=None)
    ...
    def sample(
        ...
        gnrng = np.exp(np.mean(np.log((np.max(x[:, self.stochastic_parameters], axis=0) - np.min(x[:, self.stochastic_parameters], axis=0)) / bound[self.stochastic_parameters])))
        ...
        while icall < repetitions and gnrng > peps and criter_change_pcent > pcento and proceed == True:
            ...
            param_generator = ((rep, x, xf, cx, cf, sce_vars) for rep in range(int(self.ngs)))
            ...
            if self.breakpoint == "write" or self.breakpoint == "readandwrite" and self.status.rep >= self.backup_every_rep:
                work = (self.status.rep, (x, xf), gnrng)
                self.write_breakdata(self.dbname, work)
            ...
            if self.status.rep >= repetitions:
                ...
                absolute_change = np.abs(criter[nloop - 1] - criter[nloop - kstop]) * 100
                ...
                if criter_change_pcent <= pcento:
                    print("THE BEST POINT HAS IMPROVED IN LAST %d LOOPS BY LESS THAN THE USER-SPECIFIED THRESHOLD %f" % (kstop, pcento))
        ...
        print("THE BEST POINT HAS IMPROVED IN LAST %d LOOPS BY %f PERCENT" % (kstop, criter_change_pcent))
